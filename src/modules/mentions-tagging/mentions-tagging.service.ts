import { Injectable, NotFoundException } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository } from 'typeorm';
import { Mention } from './entities/Mention.entity';
import { CreateMentionDto } from './dto/create-mentions-tagging.dto';
import { UpdateMentionDto } from './dto/update-mentions-tagging.dto';

@Injectable()
export class MentionsTaggingService {
  constructor(@InjectRepository(Mention) private readonly repository: Repository<Mention>) {}

  async create(dto: CreateMentionDto): Promise<Mention> {
    const entity = this.repository.create({ ...dto, createdAt: new Date(), updatedAt: new Date() });
    return this.repository.save(entity);
  }

  async findAll(): Promise<Mention[]> {
    return this.repository.find({ order: { createdAt: 'DESC' }, take: 100 });
  }

  async findOne(id: string): Promise<Mention> {
    const entity = await this.repository.findOne({ where: { id } });
    if (!entity) throw new NotFoundException(`Mention ${id} not found`);
    return entity;
  }

  async update(id: string, dto: UpdateMentionDto): Promise<Mention> {
    const entity = await this.findOne(id);
    Object.assign(entity, dto);
    entity.updatedAt = new Date();
    return this.repository.save(entity);
  }

  async remove(id: string): Promise<void> {
    const entity = await this.findOne(id);
    await this.repository.remove(entity);
  }
}
